...
describe('detectTransfers', () => {
  it('produces a high-confidence auto-link candidate for same-day same-amount pair', () => {
    const txs: TxWithAccount[] = [
      base({ id: 'from', accountId: 'acc-a', amountCents: BigInt(-10000), descriptionRaw: 'Transfer out' }),
      base({ id: 'to', accountId: 'acc-b', accountType: 'savings', amountCents: BigInt(10000), postedDate: '2026-03-01', descriptionRaw: 'Transfer in' }),
    ];
  ...
  it('matches within ±3 days', () => {
    const txs: TxWithAccount[] = [
      ...
      base({ id: 'to', accountId: 'acc-b', accountType: 'savings', amountCents: BigInt(5000), postedDate: '2026-03-04' }),
    ];
  ...
  it('does not match at ±4 days', () => {
    const txs: TxWithAccount[] = [
      base({ id: 'from', accountId: 'acc-a', amountCents: BigInt(-5000) }),
      base({ id: 'to', accountId: 'acc-b', accountType: 'savings', amountCents: BigInt(5000), postedDate: '2026-03-05' }),
    ];
  ...
  it('boosts confidence when description contains "transfer"', () => {
    ...
    const boosted = detectTransfers([
      ...
      base({ id: 'to', accountId: 'acc-b', accountType: 'savings', amountCents: BigInt(5000), descriptionRaw: 'Transfer in' }),
    ]);
  ...
  it('does not match same account', () => {
    const txs: TxWithAccount[] = [
      base({ id: 'from', accountId: 'acc-a', amountCents: BigInt(-5000) }),
      base({ id: 'to', accountId: 'acc-a', accountType: 'checking', amountCents: BigInt(5000) }),
    ];
    expect(detectTransfers(txs)).toHaveLength(0);
  });
  ...
  it('produces cc_payment linkType when receiving account is credit_card', () => {
    const txs: TxWithAccount[] = [
      base({ id: 'from', accountId: 'acc-a', amountCents: BigInt(-10000), descriptionRaw: 'Visa payment' }),
      base({ id: 'to', accountId: 'acc-b', accountType: 'credit_card', amountCents: BigInt(10000), descriptionRaw: 'Payment received' }),
    ];
    const candidates = detectTransfers(txs);
  ...
  it('caps ambiguous candidates below auto-link threshold', () => {
    const txs: TxWithAccount[] = [
      base({ id: 'from', accountId: 'acc-a', amountCents: BigInt(-5000), descriptionRaw: 'Transfer' }),
      base({ id: 'to1', accountId: 'acc-b', accountType: 'savings', amountCents: BigInt(5000) }),
      base({ id: 'to2', accountId: 'acc-c', accountType: 'savings', amountCents: BigInt(5000) }),
    ];
    const candidates = detectTransfers(txs);
